...
int ZoneFsBackend::Write(char *data, uint32_t size, uint64_t pos) {
  ...
  uint32_t num_zones = 32;

  uint32_t chunk_size = size / num_zones;
  uint32_t remaining_size = size % num_zones;

  std::vector<std::future<int>> write_tasks;

  for (size_t i = 0; i < num_zones; ++i) {
    uint32_t current_chunk_size = chunk_size + (i < remaining_size ? 1 : 0);
    uint64_t current_pos = pos + i * chunk_size;
    char* current_data_ptr = data + i * chunk_size;
    uint64_t current_offset = LBAToZoneOffset(current_pos);

    
    write_tasks.push_back(std::async(
        std::launch::async, 
        [this, current_data_ptr, current_chunk_size, current_pos,
         current_offset]() mutable -> int {

          
          std::shared_ptr<ZoneFsFile> file = GetZoneFile(current_pos, O_WRONLY | O_DIRECT);
          if (file == nullptr) {
            return -1;
          }

          uint32_t bytes_to_write = current_chunk_size;
          uint32_t total_written = 0;

       
          while (bytes_to_write > 0) {
            ssize_t ret = pwrite(file->GetFd(), current_data_ptr + total_written,
                                 bytes_to_write, current_offset);
            if (ret > 0) {
              bytes_to_write -= ret;
              current_offset += ret;
              total_written += ret;

              if (current_offset >= zone_sz_) {
                PutZoneFile(current_pos, O_WRONLY);
                break;
              }
            } else {
              return (ret == -1) ? -1 : ret;
