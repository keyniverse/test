...
IOStatus ZoneFile::CloseActiveZone() {
  ...
  if (!active_zone_.empty()) {
    for (auto& zone : active_zone_) { 
...
IOStatus ZoneFile::BufferedAppend(char* buffer, uint32_t data_size) {
  ...
  while (left) {
    ...
    for (auto* zone : active_zone_) {
       if (wr_size > zone->capacity_) wr_size = zone->capacity_;
    }
    ...
    for (auto* zone : active_zone_) {
      s = zone->Append(buffer, wr_size + pad_sz);
      if (!s.ok()) return s;
    }
    ...
    for (auto* zone : active_zone_) {
      extents_.push_back(new ZoneExtent(extent_start_, extent_length, zone));
    }
...
IOStatus ZoneFile::SparseAppend(char* sparse_buffer, uint32_t data_size) {
  ...
  while (left) {
    ...
    for (auto* zone : active_zone_) {
       s = zone->Append(sparse_buffer, wr_size + pad_sz);
       if (!s.ok()) break;
    }
    if (!s.ok()) return s;
...
IOStatus ZoneFile::Append(void* data, int data_size) {
  ...
  while (left) {
    ...
    for (auto* zone : active_zone_) {
      s = zone->Append((char*)data + offset, wr_size);
      if (!s.ok()) break;
    }
    if (!s.ok()) return s;
...
void ZoneFile::ReleaseActiveZone() {
    assert(!active_zone_.empty());
    for (Zone* zone : active_zone_) {
        bool ok = zone->Release();
        assert(ok);
        (void)ok;
    }
    active_zone_.clear();
...
void ZoneFile::SetActiveZone(std::vector<Zone*>& zone) {
    assert(active_zone_.empty());
    for (Zone* t_zone : zone) {
        assert(t_zone->IsBusy());
        active_zone_.push_back(t_zone);
    }
}
